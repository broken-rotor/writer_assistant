...
class KeyInformationExtractor:
    ...
    def _build_character_patterns(self) -> List[Tuple[re.Pattern, List[str], float]]:
        ...
        return [
            (re.compile(r'\b[A-Z][a-z]+\s+(?:said|thought|felt|walked|ran|smiled)\b', re.IGNORECASE), ['dialogue', 'action'], 0.8),
            (re.compile(r'\b(?:he|she|they)\s+(?:was|were|is|are)\s+\w+\b', re.IGNORECASE), ['description'], 0.6),
        ]
