...
def _validate_plot_outline_to_chapter_detail(phase_output: str, story_context: dict) -> list[ValidationResult]:
    ...
    results.append(
        ValidationResult(
            ...
            passed=has_conflict,
            message="Plot outline includes conflict or challenge" if has_conflict else
                    "Plot outline should include conflict or challenge",
...
def _validate_chapter_detail_to_final_edit(phase_output: str, story_context: dict) -> list[ValidationResult]:
    ...
    results.append(
        ValidationResult(
            ...
            message=f"Chapter has {paragraph_count} paragraphs (good structure)" if paragraph_count >= 3 else
                    "Chapter could benefit from more paragraph breaks",
            score=min(
                1.0,
                paragraph_count /
                3)))
